...
export const useCarparkStore = defineStore("carparks", {
    ...
    actions: {
        async get() {
            let data = await axios.get("https://api.data.gov.sg/v1/transport/carpark-availability");
            if (data) {
                for (let carpark of data.data.items[0].carpark_data) {
                    for (let c of rawCarparkData) {
                        if (c.car_park_no === carpark.carpark_number) {
                            let coords = proj4("+proj=tmerc +lat_0=1.366666666666667 +lon_0=103.8333333333333 +k=1 +x_0=28001.642 +y_0=38744.572 +ellps=WGS84 +units=m +no_defs", "+proj=longlat +datum=WGS84 +no_defs", [c.x_coord, c.y_coord]);
                            this.carparks.push({
                                lat: coords[0],
                                lng: coords[1],
